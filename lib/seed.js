...
module.exports = function (app, action, options, callback) {
  ...
  seed.once('complete', callback || function () {
      process.exit();
    });
...
function Seed(app) {
  ...
  eachModels(app, function (Model) {
    Model.seed = getSeedMethod(Model);
  });
...
Seed.prototype.plant = function (app, dir) {
  ...
  eachModels(app, function (Model, name) {
    global[name] = loopback.getModelByType(Model);
    debug('inject model', name, '->', Model.modelName);
  });
...
Seed.prototype.harvest = function (app, file, type) {
  ...
  if (modelNames[0] && !app.models[modelNames[0]].dataSource) {
    return app.models[modelNames[0]].once('dataSourceAttached', function () {
      setTimeout(function () {
        that.harvest(app, file, type);
      }, 1000);
    });
  }
  ...
  modelNames = modelNames.filter(function (name) {
    // filter models that has data access funcions
    return typeof app.models[name].all === 'function';
  });
